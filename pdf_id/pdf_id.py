...
class PDFId(ServiceBase):
    ...
    def execute(self, request: ServiceRequest) -> None:
        ...
        if (os.path.getsize(request.file_path) or 0) >= max_size and not request.deep_scan:
            max_size_in_mb = round(max_size/(1024**2), 1)
            section = ResultSection(f"PDF Analysis of the file was skipped because the file is too big (limit is {max_size_in_mb} MB).")
    ...
    def analyze_pdf(
        ...
        if run_pdfparse:
            ...
            carved_content: dict[str, list[tuple[str, str]]] = {}  # Format { "objnum": [{keyword: content list}}
            ...
            if carved_content:
                show_content_of_interest = self._carved_content(request, carved_content, carres)
    ...
    def _carved_content(
        self, request: ServiceRequest, carved_content: dict[str, list[tuple[str, str]]], carres: ResultSection
    ) -> bool:
        show_content_of_interest = False
        carved_extracted_shas: set[str] = set()
        carved_obj_size_limit = int(request.get_param("carved_obj_size_limit"))
        for objnum, objlist in sorted(carved_content.items()):
            for keyw, con in objlist:
